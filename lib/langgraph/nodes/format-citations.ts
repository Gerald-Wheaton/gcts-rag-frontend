 ...
 */
export async function formatCitations(state: HandbookGraphStateType): Promise<PartialHandbookState> {
  try {
    ...
    console.log(`[formatCitations] Formatting ${retrievedChunks.length} citations`);
    ...
    const citations: Citation[] = retrievedChunks.map((chunk, idx) => {
      ...
      const preview = chunk.content.length > HANDBOOK_CONFIG.citations.previewLength
        ? chunk.content.substring(0, HANDBOOK_CONFIG.citations.previewLength).trim() + '...'
        : chunk.content;
    ...
    return {
      error: `Failed to format citations: ${error instanceof Error ? error.message : 'Unknown error'}`,
    };
  }
}
 ...
 */
export function filterCitationsByAnswer(citations: Citation[], answer: string): Citation[] {
