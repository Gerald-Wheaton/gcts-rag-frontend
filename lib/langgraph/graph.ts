 ...
 *   └─ ELSE: → embedQuery → retrieveVectors → rerankChunks → synthesizeAnswer → formatCitations → END
 *
...
export function createHandbookGraph() {
  ...
  // Set entry point
  workflow.setEntryPoint(NODE_NAMES.ANALYZE_QUERY)

  // Add conditional routing after analysis
  workflow.addConditionalEdges(
    NODE_NAMES.ANALYZE_QUERY,
    (state) => {
      // Check for errors
      if (state.error) {
        return 'end'
    ...
    {
      [NODE_NAMES.REQUEST_CLARIFICATION]: NODE_NAMES.REQUEST_CLARIFICATION,
      [NODE_NAMES.EMBED_QUERY]: NODE_NAMES.EMBED_QUERY,
      end: END,
    },
  )

  // Clarification always ends (user needs to respond)
  workflow.addEdge(NODE_NAMES.REQUEST_CLARIFICATION, END)

  // After embedding, check for errors then proceed to retrieval
  workflow.addConditionalEdges(
    NODE_NAMES.EMBED_QUERY,
    (state) => {
    ...
    {
      [NODE_NAMES.RETRIEVE_VECTORS]: NODE_NAMES.RETRIEVE_VECTORS,
      end: END,
    },
  )

  // After retrieval, decide whether to rerank or go straight to synthesis
  workflow.addConditionalEdges(
    NODE_NAMES.RETRIEVE_VECTORS,
    (state) => {
      if (state.error) {
        return 'end'
      }
      ...
      // Few chunks, skip reranking
      return NODE_NAMES.SYNTHESIZE_ANSWER
    },
    {
      [NODE_NAMES.RERANK_CHUNKS]: NODE_NAMES.RERANK_CHUNKS,
      [NODE_NAMES.SYNTHESIZE_ANSWER]: NODE_NAMES.SYNTHESIZE_ANSWER,
      end: END,
    },
  )

  // After reranking, proceed to synthesis
  workflow.addConditionalEdges(
    NODE_NAMES.RERANK_CHUNKS,
    (state) => {
      if (state.error) {
        return 'end'
      }
  ...
  workflow.addConditionalEdges(
    ...
    {
      [NODE_NAMES.FORMAT_CITATIONS]: NODE_NAMES.FORMAT_CITATIONS,
      end: END,
    },
  )

  // Citation formatting always ends
  workflow.addEdge(NODE_NAMES.FORMAT_CITATIONS, END)
