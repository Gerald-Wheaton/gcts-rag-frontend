...
export async function executeHandbookPipeline(
  ...
  console.log('[Pipeline] Step 5: Synthesizing answer');
  ...
  console.log('[Pipeline] Step 6: Formatting citations');
...
export async function executeHandbookPipelineStreaming(
  ...
  const { synthesizeAnswerStreaming } = await import('@/lib/langgraph/nodes/synthesize-answer');
