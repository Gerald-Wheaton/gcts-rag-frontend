...
export const AnalyzedQuerySchema = z.object({
  ...
  entities: z.array(z.string()).describe('Key entities extracted from the query'),
  implicitFilters: z.object({
    section_type: z.string().optional().describe('Auto-determined section type filter'),
    stakeholder_groups: z.array(z.string()).optional().describe('Auto-determined stakeholder groups'),
  }),
  embedding: z.array(z.number()).optional().describe('Vector embedding for semantic search'),
  confidence: z.number().min(0).max(1).describe('Confidence score for query understanding'),
  requiresClarification: z.boolean().describe('Whether the query needs clarification'),
  clarificationReason: z.string().optional().describe('Reason why clarification is needed'),
});
...
export const RetrievedChunkSchema = z.object({
  ...
  section_path: z.string().describe('Hierarchical path in handbook (e.g., "Academic > Policies")'),
  section_type: z.string().describe('Category of the section'),
  stakeholder_groups: z.array(z.string()).describe('Who the content applies to'),
  approval_authority: z.string().describe('Who authorized this content'),
  similarity_score: z.number().describe('Cosine similarity score from vector search'),
  relevance_score: z.number().optional().describe('LLM-based relevance score (0-10)'),
...
export const CitationSchema = z.object({
  number: z.number().int().positive().describe('Citation number [1], [2], etc.'),
...
export const ConversationMessageSchema = z.object({
  ...
  citations: z.array(CitationSchema).optional().describe('Citations for assistant messages'),
});
...
export type RelevanceScoringOutput = z.infer<typeof RelevanceScoringOutputSchema>;
...
export const ConversationDocumentSchema = z.object({
  ...
  userId: z.string().default('anonymous').describe('User identifier (for future auth)'),
 ...
 */
export type StreamChunkType = 'answer' | 'citation' | 'error' | 'done' | 'clarification';
